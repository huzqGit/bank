...
COMMENT ON BANK.T_ORGAN ( 
	ORGAN_TYPE IS '是否为管理员' );
  
ALTER TABLE BANK.T_ORGAN
  ADD COLUMN ORGAN_TYPE	VARCHAR(10);

COMMENT ON BANK.T_ORGAN ( 
	ORGAN_TYPE IS '组织机构类型' ); 
	
ALTER TABLE BANK.T_ORGAN
  ADD COLUMN ORGAN_NO	VARCHAR(100);

COMMENT ON BANK.T_ORGAN ( 
	ORGAN_NO IS '组织机构编码' );
	
ALTER TABLE BANK.T_USER
  ADD COLUMN UNIT_ID	VARCHAR(40);	

ALTER TABLE BANK.T_ORGAN
  ADD COLUMN SEQ	INTEGER;

ALTER TABLE BANK.T_COOPERATION_DEBT ADD COLUMN YEARMONTH	VARCHAR(10);
COMMENT ON BANK.T_COOPERATION_DEBT (YEARMONTH IS '年月',RECEIVABLES IS '应收账款净额');

ALTER TABLE BANK.T_USER
  ADD COLUMN REMIND_CYCLE	INTEGER;

ALTER TABLE BANK.T_USER
  ADD COLUMN REMIND_DATE	DATE;
  
CREATE TABLE BANK.T_SYS_LOG (
  ...
  Alter table t_sys_log alter column log_id set 
