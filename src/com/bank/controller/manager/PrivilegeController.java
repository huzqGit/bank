...
public class PrivilegeController {
	...
	public Privilege loadPrivileges(HttpServletRequest request, HttpServletResponse response) {
	...
	public Privilege save(HttpServletRequest request, HttpServletResponse response) {
		...
		if (ADD.equals(actionType)) {//做新增操作
			try {
			...
			try {
				...
				log.error("update privilege (pk:" + privilege.getId() + ") occurs error", e);
			} catch (DataNotFoundException e) {
				log.error("delete privilege not found (pk:" + privilege.getId() + ")", e);
			}
		}
		response.setContentType("text/html;charset=UTF-8");
	    try {
	...
	public boolean delete(@RequestParam("privilegeId") String privilegeId){
		try {
			...
			log.error("delete privilege not found (pk:" + privilegeId + ")", e);
		}
		return true;
	}
	
	@RequestMapping(value = "/savePrivileges", method = RequestMethod.POST)
	public void savePrivileges(HttpServletRequest request, HttpServletResponse response) throws Exception
	{
	    ...
	    for(int i=0,l=rows.size(); i<l; i++){
	        ...
	        privilege.setId(id);
	        privilege.setPrivilegeType(row.get("privilegeType") != null ? row.get("privilegeType").toString() : "");
